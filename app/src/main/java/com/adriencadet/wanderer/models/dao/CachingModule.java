    ...
    <T extends ICachingDTO> void merge(Realm realm, List<T> freshData, List<T> oldData, Func1<T, RealmObject> converter, int cachingDurationMins) {
        ...
        while (i1 < s1 && i2 < s2) {
            ...
            if (freshObject.getId() == oldObject.getId()) { // Update
                oldObject.removeFromRealm();

                freshObject.setUpdatedAt(DateTime.now().toDate());
                realm.copyToRealm(converter.call(freshObject));
                i1++;
                i2++;
        ...
        while (i1 < s1) {
            ...
            o.setUpdatedAt(DateTime.now().toDate());
            realm.copyToRealm(converter.call(o));
            i1++;
        }
